...
def _olla() -> Family:
    return Family(
        ...
        source="用户 2026-08-02 给的现网基线 +0.01/−0.1",
        options=[
            Option("olla_db", "dB 域偏置（现网基线）",
                   formula=r"\delta_{up} = 0.01\ \text{dB}, \quad "
                           r"\delta_{down} = 0.1\ \text{dB}",
                   summary="ACK 小步加、NACK 大步减，稳态 BLER 收敛到步长比",
                   detail="步长**比例**决定稳态 BLER（0.01/0.11 ≈ 9.1%），"
                          "**绝对大小**决定收敛速度。现网基线很小，收敛慢但稳。"
                          "**MCS 是整数档，0.01 dB 常常压不动一档**——"
                          "所以短仿真里 BLER 还停在 0.16~0.22 而不是 9.1%，"
                          "未收敛时结果里会主动告警。",
                   when="默认；出正式结论用它",
                   cost="每次 ACK/NACK 一次加减"),
            Option("olla_fast", "放大步长快速收敛",
                   formula=r"\delta_{up} = 0.1, \quad \delta_{down} = 1.0",
                   summary="比例不变、绝对值放大 10 倍",
                   detail="稳态 BLER 不变（还是 9.1%），但收敛快得多。"
                          "代价是稳态附近抖得厉害。**调参试跑用它，出结论换回基线。**",
                   when="快速迭代、看趋势",
                   cost="同上"),
            Option("off", "关闭 OLLA",
