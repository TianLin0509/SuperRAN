...
**不能用 ``snr_dB - sinr_dB``。** 这个式子在教科书上成立（SNR=S/N、
SINR=S/(I+N)，相除即 (I+N)/N），但 ChannelHub 的这两个字段口径不同：

* ``snr_dB``（``internal_sim.py:2441``）= 服务小区接收功率 - 全带宽噪声
  - ``10log10(RB)``，**不含阵列增益**；
* ``sinr_dB``（几何 SINR）的信号项是 ``rx_lin · N_ant · |w^H a|^2``，
  **含阵列增益**，噪声按全带宽算。

两者相差一个 ``10log10(RB) + 10log10(N_ant·bf)`` 的定标偏差——273 RB、
64 天线时约 40 dB。相减得到的数形状对、随场景变化的趋势也对，
**只是整体偏了几十 dB**，拿它当 IoT 会把中等干扰场景报成"极高干扰"。

正确做法是只用同一口径的两个量。``sir_dB`` 与 ``sinr_dB`` 都出自
``compute_geometry_sinr_single_ue``，共用同一个 S：

    SINR = S/(I+N)、SIR = S/I
    1/SINR - 1/SIR = N/S
    IoT = (I+N)/N = (S/N)/(S/(I+N)) = SIR/(SIR - SINR)    （线性域）

于是 IoT 完全由已落盘的两个字段决定，不需要额外仿真。
"""
...
def iot_db(sinr_db: Any, sir_db: Any) -> np.ndarray:
    ...
    见模块文档——用 ``snr_dB - sinr_dB`` 会差几十 dB。
...
def interference_report(dataset_id: str) -> dict[str, Any]:
    ...
    if ul_sinr.size and ul_sir_geo.size:
        ...
        out["notes"].append(
            ...
            "上行 IoT 需要 interference.install_geometry_capture() 在生成时挂钩采集，"
            "旧数据集没有这一列。"
        )
...
def design_hint(target_iot_db: float) -> dict[str, Any]:
    ...
    **不返回保证能达标的配置。** IoT 由几何、负载、功率共同决定，
    ...
    return {
        ...
        "verification": (
            "生成后调 sw_interference_report 复核 IoT 中位数；"
...
# 上行几何 SIR 采集钩子
# ---------------------------------------------------------------------------
# ChannelHub 的 compute_geometry_sinr_single_ue 同时算出下行和上行的几何
# SIR，但 internal_sim 只把下行那个存进 ChannelSample.sir_dB，上行的
# sir_ul_db 在函数返回后就被丢掉了（ChannelSample.ul_sir_dB 存的是**测量域**
# 的导频 SIR，完全是另一个量）。没有它就算不出上行 IoT——而 IoT 本来就是
# 上行的经典指标（基站侧噪声抬升）。
#
# 这里包一层把整份返回值暂存下来，生成侧在拿到 sample 之后立刻取走。
# **包装函数自带自检**：把暂存里的 dl_sinr_avg / sir_dl_db 和 sample 自己的
# sinr_dB / sir_dB 对一遍，对不上就说明"一次调用 <-> 一个样本"的假设破了
# （比如 ChannelHub 改成批量算），这时宁可不给上行 IoT，也不给错的。
...
def install_geometry_capture() -> bool:
    """给 ChannelHub 的几何 SINR 函数挂一层暂存。幂等，返回是否已挂上。
    ...
    try:
        ...
        if getattr(orig, "_sw_wrapped", False):
            _installed = True
            return True
        ...
        wrapper._sw_wrapped = True  # type: ignore[attr-defined]
...
def take_ul_geometry_sir(sample: Any) -> float:
    """取走上一次几何 SINR 计算里的上行 SIR，取不到或对不上时回 nan。

    自检：暂存的下行量必须与 sample 自己报的 ``sinr_dB`` / ``sir_dB`` 一致
