...
def power_delay_profile(
    ...
    *,
    ...
    返回的 power 是**线性功率、未归一化**；时延轴由 RB 带宽推出：
    ...
    # 沿 RB 轴 IFFT 到时延域，乘 sqrt(RB) 保持 Parseval 能量守恒
    h_delay = np.fft.ifft(h, axis=1) * np.sqrt(n_rb)
    p = np.abs(h_delay) ** 2
    ...
    if total > _EPS:
        w = flat / total
        mean_tau = float((w * delays_s).sum())
        rms_ds = float(np.sqrt(((delays_s - mean_tau) ** 2 * w).sum()))
    else:
        mean_tau = 0.0
        rms_ds = 0.0
