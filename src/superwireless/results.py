...
由 sw_export_eval_template 生成。**把 my_algorithm 换成你自己的算法**，
其余不用动。跑完会把两个臂的结果注册回 superwireless，
之后用 sw_compare_results 过门 2 / 门 3。
...
import numpy as np
from superwireless import load, results
...
print(f'  sw_compare_results("{{art_a.result_id}}", "{{art_b.result_id}}")')
'''
...
def eval_template(dataset_id: str, *, metric: str = "spectral_efficiency") -> dict[str, Any]:
    ...
    return {
        ...
        "how_to_use": [
            ...
            "运行它，会打印两个 result_id",
            "把两个 result_id 交给 sw_compare_results，过门 2 与门 3",
        ],
        "guardrails": [
