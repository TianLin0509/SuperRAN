...
用法::
    ...
    # → pr.prereg_id = "pr_a1b2c3d4"，把它传给 sw_generate
...
def lock(
    *,
    ...
    返回的 ``prereg_id`` 传给 ``sw_generate``，会一起存进数据集摘要，
    之后 ``sw_compare_results`` / 门 3 就能判断用的指标是不是当初定的那个。
...
def classify(pr: Prereg | None, metric: str) -> dict[str, Any]:
    ...
    if pr is None:
        return {
            ...
            "how_to_claim": "下次生成前先 sw_lock_analysis，把主指标写下来",
        }
    if not verify(pr):
        return {
            "status": "tampered",
