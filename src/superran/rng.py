...
跑 8 次 ``sr_system_sim``（数据在 ``measurements/seed_variance.json``）：
...
class RngBook:
    ...
    **重复实验换 replication，不换 master_seed。** ns-3 手册的理由是两个随机
    ...
    def __post_init__(self) -> None:
        for name, value in (("master_seed", self.master_seed),
...
def replications(master_seed: int = 0, n: int = 8, *, start: int = 0) -> list[RngBook]:
    ...
    **A/B 两臂拿同一个返回值就是公共随机数**——两边第 k 次重复用的是同一套
    ...
    """
    base = RngBook(master_seed, start)
    if (isinstance(n, (bool, np.bool_))
            or not isinstance(n, (int, np.integer)) or int(n) < 1):
        raise ValueError(f"重复次数至少 1 次，收到 {n}")
    return [RngBook(base.master_seed, base.replication + i) for i in range(int(n))]


def derive_generate_seed(master_seed: int = 0, replication: int = 0) -> int:
    """给 ``sr_generate`` 用的 ChannelHub 种子，从 ``channel`` 流派生。

    **信道流和话务流分开的价值就在这**：改话务参数不会连带换掉信道实现，
    否则 A/B 比的是"算法差异 + 换了一批信道"。ChannelHub 只认一个 int，
    所以这里折成整数，但派生路径仍然是同一套 ``SeedSequence``。
    """
    return RngBook(master_seed, replication).integer_seed("channel")
