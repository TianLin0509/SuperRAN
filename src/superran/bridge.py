...
用户点一下「应用到仿真」就结束，agent 这边 ``sr_await_config`` 正等着。
...
def _sanitize(raw: Any, allowed: frozenset[str]) -> tuple[bool, dict[str, Any], str]:
    ...
    不做这一步，一个越界的键就能悄悄进入后面的 ``sr_revise``。
    """
...
# ---------------------------------------------------------------------------
def enabled() -> bool:
    return os.environ.get("SUPERRAN_NO_SERVE", "") not in ("1", "true", "TRUE")
...
def open_url(target: str) -> bool:
    ...
    """
    if os.environ.get("SUPERRAN_NO_BROWSER", "") in ("1", "true", "TRUE"):
        return False
    try:
