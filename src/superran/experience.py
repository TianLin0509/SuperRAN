...
（频选或 MU 打开时一个 TTI 会服务多个用户，实测默认 1.14、开 MU 1.86）。
...
52.915 vs 53.003 Mbps，差 0.17%；但它们的发送字节分子同源，接近只能检查
...
def simulate_experience(
    ...
    def _retx_now(slot_type: str) -> dict[int, _HarqTb]:
        ...
        一个 UE 一个 TTI 只发一个 TB（NR 里一个 DCI 一个 TB），所以给
        plan builder 的仍然是"每 UE 至多一个"的视图——它们一行都不用改。
        完整的在途状态留在 ``harq_inflight`` 里。
        """
        out: dict[int, _HarqTb] = {}
        for ue, slots in harq_inflight.items():
            ready = [tb for tb in slots.values()
                     if tb.state == "retx_ready" and tb.slot == slot_type]
            if ready:
                out[ue] = min(ready, key=lambda tb: (tb.first_tti, tb.harq_id))
        return out

    def _can_send(ue: int, slot_type: str) -> bool:
        """本时隙这个 UE 发得出东西吗：有空闲进程能发新传，或有重传就绪。"""
        if harq_free_ids[ue]:
            return True
        return any(tb.state == "retx_ready" and tb.slot == slot_type
                   for tb in harq_inflight[ue].values())
    ...
    for tti in range(int(sys_cfg.num_tti)):
        ...
        tr.step(tti)
        # UE 侧 CQI 按 CSI 报告周期上报，随后回写进本快照。**放在调度判决之前**：
        ...
        for _u_fb in range(n_ue):
            for _id_fb in sorted(harq_inflight[_u_fb]):
                ...
                _pending_fb.feedback.apply(
                    rank_controller=rank_ctl, su_olla=olla_db,
                    mu_olla=mu_olla_db,
        ...
        if in_measurement:
            # "被在途反馈挡住"的口径没变：有数据、进程全满、且在途的每个 TB
            # 都还在等反馈。``harq_max_processes=1`` 时与历史计数逐值一致。
        ...
        blocked_data = blocked_data or any(
            tr.has_data(u) and not _can_send(u, slot)
            for u in range(n_ue))
        ...
        for group_idx, grant in enumerate(final_grants):
            ...
            for side, u in enumerate(grant.users):
                ...
                if is_retx:
                    ...
                    # 要等反馈回来才释放该进程；payload 已在首传时离开队列，
                    # 末次失败只进 residual_bler，不回队列。
                ...
                if sched.olla_enabled and not is_retx:
                    ...
                    olla_effective_tti = (
                        tti + int(feedback_offsets[tti % pattern_len]))
                if not is_retx:
                    # 首传 ACK/NACK 占住本次分配的 HARQ 进程，直到反馈到达；
                    # 同 UE 的其他空闲进程仍可发新 TB。outcome 在发送时抽样，
                    # 但只能由下一轮顶部的 due-event 路径交给 OLLA 与 RankController。
                    _new_id = harq_free_ids[u].pop(0)
                    harq_inflight[u][_new_id] = _HarqTb(
                        harq_id=int(_new_id),
    ...
    cell = {
        ...
        "harq_ues_with_inflight_tb_at_end": int(np.count_nonzero(pending_measured)),
    ...
    notes: list[str] = [
        ...
        ("ACK/NACK 搭发送之后第一个 U 时隙回传，OLLA 更新与重传资格从该 U 之后"
         ...
         "（S 上发的 TB 要等下一个 S），两个约束取交集。每个在途 TB 只占自己的 "
         f"HARQ 进程；当前每 UE 上限 {int(_max_proc)}。k2、PUCCH 资源与完整标准 K1 查表未建模。"
