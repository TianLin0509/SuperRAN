...
这是 SuperRAN 唯一不可替代的部分。参数转发谁都能写，但"哪几件事
...
class DesignQuestion:
    ...
    **给选项，不要只抛问题。** 开放式提问会让用户卡壳——"你要跟什么比"
    这种问题，很多人第一反应是答不上来，但看到"理想 CSI 的逐 RB 特征预编码参考"
    ...
    for q in (
        DesignQuestion(
            ...
            options=[
                Option("type1_2", "3GPP Type I 或 Type II 码本",
                       ...
                       recommended=True),
                Option("svd_bound", "理想 CSI 的逐 RB 特征预编码",
                       "乐观的等功率参考；真正 Shannon 容量上界需另算注水容量"),
        ...
        DesignQuestion(
            ...
            why=(
                ...
                "流程上先跑 20 个样本做试点量 σ_d，再算正式样本数（`sr_sample_size`）。"
            ),
            options=[
...
_SCENARIO = Decision(
    ...
    why=(
        ...
        "带 _LOS 后缀的版本显式条件化为视距：LOS 概率固定为 1，同时使用视距路损"
        "和 LOS CDL/TDL 剖面。历史上的 _NLOS 名称表示基础随机场景，仍按 38.901 的"
        "距离概率逐链路抽取 LOS/NLOS 并切换生效模型。两者不是只改一个输出标签。"
    ),
    options=[
...
_LOAD_SWEEP = Sweep(
    ...
    why=(
        ...
        "两组会生成一模一样的数据（sr_plan 的 guards 会拦）。"
        "生成后用 sr_interference_report 复核两组的 IoT 确实拉开了。"
    ),
)
...
TASK_PROFILES: tuple[TaskProfile, ...] = (
    ...
    TaskProfile(
        ...
        pitfalls=(
            ...
            "先用 sr_probe_scenario 确认 IoT 真的拉开了，再花时间跑正式对比。",
        ),
...
def next_round(
    ...
    **设计层和参数层没有先后依赖**，早先把它们拆成两轮纯粹是分类整齐，
    ...
    由 ``sr_sample_size`` 算（见 CONCLUSION_TEMPLATE 那一节）。
        ...
        issues.append(
            {
                ...
                "suggestion": "用 sr_design_interference 看各旋钮的实测作用方向",
            }
        )
