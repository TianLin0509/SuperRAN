...
_EDITABLE: tuple[tuple[str, str, str, Any, str], ...] = (
    ...
    ("rank_gain_factor_reduce", "Rank 降档谱效比门限", "number", (0.5, 3.0, 0.05),
     "含义取决于 rank_switch_rule：spec_asymmetric 下 ≥1 表示降 rank 立即生效，"
     "unified_ratio 下 >1 表示降 rank 也要那么多余量"),
    ("rank_switch_rule", "Rank 切换判据写法", "select",
     ["spec_asymmetric", "unified_ratio"],
     "两份现场来源对降档写法不一致，且让上面那个常数含义相反；必须显式选"),
    ...
    ("srs_period_ms", "SRS 最短候选周期 ms", "select", [10.0, 20.0, 40.0],
     "默认从10 ms起；本色资源不足时全局自动升到20/40 ms"),
    ("srs_hopping", "SRS 跳频", "select", ["on", "off"],
     "38.211 C_SRS=63：每跳 16 RB、17 跳扫完全带。**老化的主导项**"),
    ("srs_resource_allocation", "SRS 资源分配", "select", ["on", "off"],
     "2T4R双腿、4 CS、17频域相位；加粗BBL叶子不进入普通资源池"),
    ("srs_period_adaptive", "SRS 周期自适应", "select", ["on", "off"],
     "选择全局最短可容纳周期；关闭后容量不足硬失败"),
    ("srs_pci_mod3", "服务小区 PCI 模 3", "select", [0, 1, 2],
     "只允许使用本色普通H叶子；禁止跨颜色借资源"),
    ...
    ("cqi_filter_lambda", "CQI 滤波系数 λ", "number", (0.01, 1.0, 0.01),
     "一阶 IIR：CQI ← CQI + λ(最新测量 − CQI)。λ=1 不滤波；默认 0.25 已对照"
     "现场 CQI 仿真核对确认（2026-09-02）"),
...
_SIM_DEFAULTS: dict[str, Any] = {
    ...
    "rank_switch_rule": "spec_asymmetric",
    ...
    state = {
        ...
        "impact": {
            ...
            "link": ["precoder", "power_constraint", "neighbor_prb_util",
                     ...
                     "srs_hopping", "srs_resource_allocation",
