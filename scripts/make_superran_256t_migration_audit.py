...
def render() -> str:
    ...
    critical = {
        ...
        "SuperRAN native.py": ROOT / "src" / "superran" / "native.py",
        "SuperRAN physical.py": ROOT / "src" / "superran" / "physical.py",
        "SuperRAN spec38901.py": ROOT / "src" / "superran" / "spec38901.py",
    ...
    replacements = {
        ...
        "__MSG_HEAD__": "not-applicable-first-party-core",
        "__MSG_BRANCH__": "dependency-removed",
...
<section id="bugs"><div class="section-head"><span class="num">07</span><h2>深潜过程中发现并当场修掉的问题</h2></div><div class="table-wrap"><table><thead><tr><th>问题</th><th>后果</th><th>修复 / 反向验证</th></tr></thead><tbody><tr><td>经典 64T 与 256T 两种展平顺序</td><td>跨阵列复用 codebook、数据集或模型时，同一个 row 代表不同物理端口。</td><td>新 64T/256T 统一 <code>pol_h_v + top_to_bottom</code>；legacy 只允许显式读取，并用置换等价门锁定。</td></tr><tr><td>未知 64 端口面板被按预置 64T 猜测</td><td>自定义 64 端口阵列会被静默套上错误物理顺序。</td><td>只有未显式给 panel 时才按端口数推 profile；显式未知面板保持 custom/legacy，测试防止未来再猜。</td></tr><tr><td>产品 PMI 在 fetch 子进程运行时导入 MSG</td><td>开发环境能跑，独立交付子进程报 <code>ModuleNotFoundError</code>。</td><td>索引与 Type-I 置换下沉为 SuperRAN 自包含 helper；完整 E2E 中文 PMI/SRS/PDP/geometry 重跑通过。</td></tr><tr><td>Sionna 内置 cross 极化顺序相反</td><td>内置 <code>[−45°,+45°]</code> 会与预置 p0=+45°、p1=−45° 对调。</td><td>显式注册 <code>[+π/4,−π/4]</code> pattern，并检查返回的 port/element/polarization 三元组。</td></tr><tr><td>PF 用 fullband best_se 记账</td><td>1 RBG 小包用户被约 17 倍过罚，后续更难获调度。</td><td>默认 <code>scheduled_tbs</code>；故意切回 legacy 时必须观察到 credit 大于实际 grant。</td></tr><tr><td>TBS 用除法估 RBG 数</td><td>量化 TBS 非线性，约 1% 偏差就可能少给 1 RBG，包发不完。</td><td>全表 <code>searchsorted</code>；单调性扫描覆盖 MCS/rank/RBG。</td></tr><tr><td>6° 是匿名默认值</td><td>手册无法解释来源，也无法审计不同下倾是否真改变 F。</td><td>新增 <code>DEFAULT_ELECTRICAL_DOWNTILT_DEG</code>，允许用户覆盖并写入 calibration/hash；文档明确它是工程基线。</td></tr><tr><td>外部 editable venv 污染</td><td>直接运行可能从别的 editable checkout 导入外部物理包，产生假绿。</td><td>first-party source 随 SuperRAN editable 安装；导入阻断器与 bogus external root 反向验证证明不再选择外部实现。</td></tr><tr><td>生成手册后源码变化</td><td>静态 HTML 容易陈旧，旧的 per-ray 归一与后端状态曾留在文字里。</td><td><code>--check</code> 对当前树重建比较；专题页同步改成当前代码行为，并重新做浏览器 QA。</td></tr></tbody></table></div></section>
