...
<section id="boundaries">
  ...
  <div class="grid two">
    ...
    <div class="card"><h3>下一阶段模型项</h3><ul><li>标准 HARQ RV/LLR/process timing；当前每 TB 最多一次 IR/CC 的 BLER 级抽象。</li><li>经链路级曲线标定的 EESM/MIESM；当前跨 RBG/rank stream 做 dB 平均。</li><li>完整 Type-I single-panel 端口/面板映射；当前工程 beam-column baseline。</li><li>CSI 报告本身的额外 feedback latency 仍为 0（HARQ 的 ACK/NACK 时延已按 TDD 图案建模）；CQI filter 已是一阶 IIR，但 λ 仍是工程默认、待现场标定。</li><li>RI/PMI 的 report-side hold；当前 rank 由显式策略给出（默认固定 rank2，可选按周期自适应），不是终端上报的 RI。</li><li>逐邻区联合调度/负载；当前聚合 scalar load。</li><li>完整 DMRS/PTRS/CORESET 开销；当前显式 12 data symbols 与 S-slot 0.7。</li><li>rank-2 子空间级 SUS；当前 dominant-vector correlation。</li></ul></div>
  </div>
