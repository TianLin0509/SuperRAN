...
FORMULA_SPECS.update({
    ...
    "F_PCI_MOD3": FormulaSpec(
        "PCI 模3把物理小区标识压缩成三种资源偏好颜色",
        "先对 NR 物理小区标识取除以 3 的余数，得到 0/1/2 三类。当前工程预置先用本类颜色，再按循环顺序使用另外两类；这是一种小区间干扰协调策略，不是 3GPP 强制规定的 SRS 资源表。",
        (("N<sub>ID</sub><sup>cell</sup>", "NR Physical Cell ID；标准定义的物理小区标识。"),
         ...
         ("Π<sub>c</sub>", "该小区的颜色搜索顺序：本色优先，资源不足再溢出到后两色。"),
         ("c", "被配置 SRS 资源的服务小区。")),
    ),
    "F_SRS_POOL_CAPACITY": FormulaSpec(
        "基础 SRS 资源池的 4-port UE 容量",
        "容量来自可选时域 offset、slot 内 symbol、comb offset 和循环移位块四个正交维度的乘积。多端口 UE 要占一组连续循环移位，因此端口越多，同一时频叶子能容纳的 UE 越少。",
        (("N<sub>UE,max</sub>", "同一小区、同一周期层级下可分配的最大 UE 数。"),
         ("N<sub>offset</sub>", "该 SRS 周期内互不等价的 slot offset 数。"),
         ("N<sub>sym</sub>", "可用的 slot 内 SRS symbol 数；当前普通 H 表为 4。"),
         ("N<sub>comb</sub>", "comb offset 数；comb-2 下为 2。"),
         ("N<sub>CS</sub>", "可用循环移位总数；当前基础表为 8。"),
         ("N<sub>port</sub>", "一个 UE 同时占用的 SRS port/循环移位数，当前支持 1/2/4。"),
         ("floor", "向下取整，保证不会给端口块分配半个循环移位。")),
    ),
...
DETAIL_SPECS.update({
    "srsallocation": DetailSpec(
        ...
        principles=(
            ...
            "当前三色首先绑定 <strong>symbol + comb</strong> 时频叶子，而不是循环移位。原因是同小区可保持较好的时间对齐，循环移位适合在一个叶子下复用；跨小区存在传播时延、TA 误差和多径，CS 正交可能受损，所以相邻 PCI 类先避开同一 symbol/comb，再把 CS 留给本小区扩容。",
            "普通 H 表对每个 SRS 机会使用同一 4×2 着色：symbol10 的 comb0/1 为 0/1，symbol11 为 2/0，symbol12 为 1/2，symbol13 为 1/0。实现从靠近 slot 尾部的 symbol13 向前搜索，因此轻载 PCI0/1/2 三小区的首个 4-port UE 分别落到 symbol13/comb1、symbol13/comb0、symbol12/comb1，三者 CS 都可取 0..3 而仍不发生时频碰撞。",
            "颜色是偏好，不是永久分区。10 ms、4-port 全池容量为 32，其中首选色容量并不相等：颜色0有 12、颜色1有 12、颜色2有 8 个叶子容量。超过本色容量后必须向后续颜色溢出，因此“PCI 模3零碰撞”只在资源未溢出的负载区间成立。仿真应同时报告色内占用、spill-over 和碰撞，而不是只报 PCI 配置。",
            "周期档位是在容量、上行开销与 CSI 新鲜度之间做交换。参考表从档位0的 10/20/40/80 ms 逐步收缩到档位4的 160 ms：周期越长，可用 offset 越多、支持 UE 越多且平均开销越低，但每个 UE 的信道刷新越慢。当前基础实现尚未做按 UE 数自动选档，只接受 10/20/40 ms 的直接请求。",
            "资源分配和频域跳频是两层。period/offset/symbol/comb/CS 决定某次 SRS 是否与别人可分离；17-hop 决定这次 SRS 探哪个 16-RB RBG，以及多久才能扫完全带。当前三种 PCI 颜色共用同一 hopping 顺序，因此不能把 PCI 模3写成已经完成了频域小区间错开。",
            "跨周期资源是否会在未来相遇必须用同余关系。10 ms offset7 与 20 ms offset27 数字不同，却会周期性重合；相反不满足同余的两条时间序列永不同时出现。只有时间相遇、symbol 相同、comb 相同、CS 交集非空且频域重合，基础模型才判碰撞。",
        ...
        implementation=(
            ("冻结表驱动真相源", "<code>PCI_MOD3_COLOR_BY_SYMBOL_COMB</code> 逐格保存普通 H 的 4×2 表；profile id 升为 v2。代码、测试和文档都从这份表读取，不再用线性序号暗中生成另一张颜色表。"),
            ("建立 UE 请求", "请求固定 UE/cell/PCI mod3/周期/port 数与 hopping 状态。固定 100 MHz 资源分配只接受 10/20/40 ms，port 接受 1/2/4；5 ms 仍可用于不启用本资源表的链路级老化研究。"),
            ("生成时频叶子", "展开周期内所有 slot-phase-7 offset、symbol13→10 和 comb0/1；先由表查叶子颜色，再在每个叶子下展开 CS block。颜色不随 CS 改变。"),
            ("按 PCI 颜色排序", "PCI0 按 0→1→2、PCI1 按 1→2→0、PCI2 按 2→0→1；排序只改变候选优先级，不删除其他颜色，因此资源池容量保持不变。"),
            ("检查周期碰撞", "与同小区已用资源做 period gcd 同余、symbol、comb 和 CS 交集检查。没有空闲叶子就硬失败，不静默改周期、降 port 或复用冲突资源。"),
            ("进入 CSI 时间轴", "assignment 的 <code>offset_ms</code> 传给 <code>rbg_lag_snapshots()</code>；最近可用测量改为 offset+n·period，分配差异会真实改变逐 UE CSI 陈旧时长。"),
            ("留存用户级证据", "每个 UE 返回 period/offset/symbol/comb/CS/color/spill tier 与 hopping profile；小区汇总返回颜色表和未建模项，使用者可从 KPI 反查到导频资源。"),
            ("运行正反例", "测试逐格锁定颜色表、验证同一时频叶子的两个 CS block 颜色相同，并比较三小区同色3/3碰撞与0/1/2三色0/3碰撞。"),
        ),
        example_title="PCI 100/101/102 三个相邻小区怎样选出三份不同 SRS 资源",
        example=(
            "<p>三个小区的 PCI 分别为 100、101、102，所以颜色依次是 1、2、0。它们都请求 10 ms、4-port，且首个可用 offset 都是 slot7=3.5 ms；不同之处来自 symbol/comb 表。颜色1从 symbol13/comb0 取 CS0..3，颜色2从 symbol12/comb1 取 CS0..3，颜色0从 symbol13/comb1 取 CS0..3。三者时间相同、CS 也相同，但 symbol/comb 至少有一维不同，所以基础判据下不碰撞。</p>"
            "<p>若强制三个小区都按颜色0排序，它们都会先取 symbol13/comb1/CS0..3，三对组合全部相撞，每个 UE 看见两个等功率污染者，I/S=2、加 N/S=0.01 后 LS NMSE proxy=2.01。恢复0/1/2三色后碰撞变0/3、proxy=0.01。该例只证明首选资源表的方向；当某色资源耗尽并溢出时，必须重新统计碰撞。</p>"
            "<p>同一小区的第二个4-port UE则可以留在同一 symbol/comb，改用CS4..7，因为同站定时更可控。这个对比正好说明：<strong>CS 是本小区复用维度，PCI 颜色优先保护跨小区时频维度</strong>。</p>"
        ),
        checks=(
            ("表逐格一致", "symbol10..13 × comb0/1 的八个颜色逐值等于 0,1,2,0,1,2,1,0；文档从同一常量生成。"),
            ("颜色与 CS 解耦", "同一时频叶子下 CS0..3 与 CS4..7 必须拥有同一 resource_color；旧版按 linear index 给 CS 不同颜色的写法由反向测试禁止。"),
            ...
            ("模 3 方向", "轻载三小区不仅 collision-pair rate 从100%降到0，三份首选 assignment 的 symbol/comb 也必须互异，而不是只靠不同 CS 过门。"),
            ("公开边界", "每份 assignment/summary 都写 P-H/F、BWP2、根序列、非理想正交和波形级污染未建模。"),
        ),
        pitfalls=(
            "把 PCI 模3写成3GPP强制资源表。标准定义资源字段，具体三色映射是工程预置。",
            "把不同循环移位涂成不同 PCI 颜色，表面上三小区不碰撞，实际上跨小区定时误差可能破坏 CS 正交。",
            ...
            "把参考档位0..4写成已经自动实现；当前只直接接受10/20/40 ms，80/160和自动选档仍是缺口。",
