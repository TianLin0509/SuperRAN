...
DETAIL_SPECS.update({
    "raytracing": DetailSpec(
        ...
        source_paths=("src/superran/scenes.py", "src/superran/scene_assets.py", "src/superran/scenario.py", "src/superran/channelhub.py", "src/superran/generate.py", "src/superran/loader.py", "tests/test_raytracing.py", "tests/test_interference.py", "tests/test_physics_contract_extensions.py"),
    ),
})
...
DETAIL_SPECS.update({
    ...
    "schedulerp0": DetailSpec(
        ...
        principles=(
            ...
            "排序度量与分配策略是<strong>两件事</strong>。PF 用 <code>TBS/R_avg</code> 追长期公平，EDF（包长感知）用 <code>TBS/Buffer</code> 追短期排空：缓冲区小 + 信道好的用户先走，一次传完就释放资源。分母换成当前队列后度量变成无状态的，不必维护历史平均。<code>qos_pf_edf</code> 按蓝本原式 <code>((1−w)·s·EPF + w·EDF)×w(priority)</code> 在两者之间连续可调。**两个分量不同量纲**：EPF 是 bytes^β/bytes^α，EDF 是无量纲比值，标定系数 <code>s</code> 不配平时名义 w=0.5 实际可能只有 0.002，因此结果必须回报 <code>effective_edf_share</code>。",
            "EDF 的定价是公平性，而且必须被量出来。24 UE 饱和实测：Jain 从 PF 的 0.4707 掉到 0.3032，代价是 2 个 UE 被完全饿死（served=0），换来的小包即时服务比例只从 0.7665 升到 0.7891。收益这么小是因为 SuperRAN 的按需 RBG 分配早就让小包在 PF 下也基本即时服务。想要 EDF 的取舍又不想饿死人，就把 <code>epf_scale</code> 标定到分量可比（本例 1e-4），混合模式能拿到 Jain 0.3096 且零饿死。",
        ...
        checks=(
            ...
            ("混合权重不许假装生效", "<code>epf_scale=1.0</code> 时 <code>effective_edf_share&lt;0.01</code> 并给出退化告警；标定到 1e-4 后占比 &gt;0.5 且不再饿死任何用户。"),
