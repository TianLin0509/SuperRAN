...
DETAIL_SPECS.update({
    ...
    "bfgain": DetailSpec(
        ...
        principles=(
            "BF Gain 是基站决策时的可知量，不是事后用 <code>h_true</code> 算出的实际增益。两套权都在同一份 <code>h_prec</code> 上设计和评估：TX 是实际要发的权（默认 SVD），PMI 是 UE 上报 CQI 时的 Type-I-style 宽带参照权。如果 SRS 估计陈旧，两边都只能看陈旧 CSI。",
            "公平对照要求同一 rank、同一总功率、同一 EBF/PEBF/NEBF 模式、同一每流功率、同一损伤参考面和同一经典 MMSE 接收机；唯一允许改变的是预编码权。NEBF 等每天线处理会改变权的物理矩阵，因而其功率利用和正交性影响本来就应进入 BF Gain。",
            "聚合顺序不能换。每条流先得到逐 RB 线性 post-MMSE SINR；RBG 内在线性域平均 RB，再转 dB；RBG 内各流和全带各 RBG 最后在 dB 域算术平均。这是当前单码字宽带工程口径，不冒充已标定的 EESM/MIESM。",
            "<code>bf_gain_user_db</code> 进入 MCS 链；<code>bf_gain_true_user_db</code> 只在真实当前信道上重算以便看预测误差。老化时两者可以相差很大，差值通过实际 BLER/ACK/NACK 进入 OLLA，不允许当次偷回发送决策。",
        ),
        implementation=(
            ("冻结对照条件", "从同一 <code>h_prec</code> 构造 SVD/actual-TX 权和宽带 PMI 权，两者强制相同 rank，用 <code>equal_power_weights</code> 施加同一功率约束。"),
            ("算逐 RB/流 SINR", "把两个物理权分别打到 gNB CSI，用经典 MMSE 闭式得到 <code>s_tx</code> 和 <code>s_pmi</code>。"),
            ("聚合并作差", "<code>rbg_sinr_db</code> 完成 RB 线性平均与dB/流聚合；逐 RBG 差保存为 <code>bf_gain_rbg</code>，全带差保存为 <code>bf_gain_db</code>。"),
            ("分离决策与审计", "<code>sr_tdd_mcs</code> 只把 gNB CSI 增益加到 CQI 门限；同一权在 <code>h_true</code> 上的差另报 <code>bf_gain_true_user_db</code> 和 prediction error。"),
        ),
        ...
        example=(
            ...
            "<p>但事后真值审计 <code>bf_gain_true_user_db</code> 会不同。一个固定回归样本中，gNB 预测都是 3.9446 dB；完全匹配真值上审计为 3.9446 dB，换一份真值后审计只有 0.8380 dB。后一臂 prediction error 是 −3.1066 dB，这个误差只能由当次误码和后续 OLLA 学到。</p>",
        ),
        checks=(
...
FORMULA_SPECS: dict[str, FormulaSpec] = {
    ...
    "F_BF_STREAM": FormulaSpec(
        ...
        (("γ<sub>f,k</sub><sup>(x)</sup>", "权 x 在 RB f、第 k 流上的线性 post-MMSE SINR。"),
         ...
         ("P/r", "总功率 P 在 r 条流之间等分后的每流功率。"),
         ("H<sub>gNB,f</sub>", "该时刻 gNB 可用的信道；可能是 SRS 估计或陈旧 CSI，不是当前 h_true。"),
         ("W<sub>x,f</sub>", "分支 x 的预编码方向，已经 EBF/PEBF/NEBF 功率约束处理。"),
         ("G<sub>x,f</sub>", "有效流信道 H<sub>gNB,f</sub>W<sub>x,f</sub>。"),
    ...
    "F_TX_SINR": FormulaSpec(
        ...
        "CQI 先查内部离散表得基础 MCS 及其 SINR 门限，再加波束赋形增益得到发送侧预测 SINR。OLLA 不进入该 SINR，而是在反折 MCS 后以连续 MCS offset 叠加。",
