...
DETAIL_SPECS.update({
    "raytracing": DetailSpec(
        ...
        principles=(
            ...
            "<strong>确定性引擎注定重复时要硬失败，而且要拦全。</strong>RT 没有随机源，CDL 靠 <code>rng_small</code> 每轮重画小尺度实现这条路它走不了。三种配置必然产出重复矩阵，入口全部硬拦：① <code>static</code> + 多轮——几何固定、时间轴又都是 <code>[0, n_time×dt)</code>，<strong>给多普勒也救不回来</strong>（上一版守卫只拦零速度，漏掉了这条）；② <code>num_slots_per_sample&gt;1</code> + 零多普勒——时间相位恒为 1，样本内部 N 个 slot 逐位相同；③ 非 static + <code>num_slots_per_sample&gt;1</code>——父类每轮只前移<strong>一个</strong> <code>sample_interval_s</code> 而样本横跨 <code>n_time</code> 个间隔，相邻两轮窗口重叠（n_time=8 时重叠 7/8，16 个输出只有 9 个独特时刻），下游还会直接展平当独立快照。第 ③ 条要改的是父类的轨迹时钟、跨两个引擎，需要维护者定案；在定案前 RT 不产出这种数据。",
