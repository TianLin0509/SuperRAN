...
DETAIL_SPECS.update({
    ...
    "schedulerp0": DetailSpec(
        ...
        principles=(
            ...
            "EDF 的定价是公平性，而且必须被量出来。24 UE 饱和实测：Jain 从 PF 的 0.4707 掉到 0.3032，代价是 2 个 UE 被完全饿死（served=0，且都是全小区最差的链路：−1.87 / −3.00 dB），换来的小包即时服务比例只从 0.7665 升到 0.7891。收益这么小是因为 SuperRAN 的按需 RBG 分配早就让小包在 PF 下也基本即时服务。想要 EDF 的取舍又不想饿死人，就把 <code>epf_scale</code> 标定到分量可比（本例 1e-4），混合模式能拿到 Jain 0.3096 且零饿死。",
