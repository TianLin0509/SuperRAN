...
DETAIL_SPECS.update({
    ...
    "mu": DetailSpec(
        ...
        principles=(
            ...
            "因此 MU 一律读同一张 pair 表（<code>mu_accounting=\"pair_table\"</code>，唯一口径）。矩阵运算全部留在建表阶段，主循环只查表：实测约 3.8 ms/pair/快照，12 UE × 40 快照约 10 s，与主循环十万 TTI 的开销相比可以忽略。历史的标量口径降级为 <code>se_ratio_legacy</code>，仅用于复现旧结果，选用时会写进结果 notes，并明说结果系统性乐观。两个口径不可拼在同一张趋势图里。",
