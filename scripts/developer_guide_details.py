...
DETAIL_SPECS.update({
    "raytracing": DetailSpec(
        ...
        principles=(
            ...
            "<strong>时间轴要跨样本连续。</strong>同一个 UE 的第 k 轮样本从 <code>k × sample_interval_s</code> 起算，与父类挪 UE 位置用的是同一个时钟。写成每个样本都从 t=0 重来时，静止 UE 的第 2、3、4… 个样本与第 1 个<strong>逐位相同</strong>——样本数涨了、信道实现数没涨，跨快照变化、CSI 老化、协方差与 rank/MCS 分布全被这批重复矩阵做假。轮次必须在父类<strong>算这个样本之前</strong>设好；写成 <code>for i, s in enumerate(super().iter_samples())</code> 会让信道用上一轮的时间原点而 meta 记着这一轮，数字自洽、物理不对。",
            "<strong>确定性引擎注定重复时要硬失败。</strong>射线追踪没有随机性：UE 不动（<code>mobility_mode='static'</code>）又没有多普勒（<code>ue_speed_kmh=0</code>）时，多出来的轮次必然逐位重复。适配层在 <code>iter_samples</code> 入口直接报错并给出三条出路（给速度、开移动、降样本数），不产出重复矩阵让人当独立实现用。",
        ...
        checks=(
            ...
            ("样本不重复", "同一 UE 相邻两轮的 <code>h_serving_true</code> 必须不同（实测 NMSE −18 dB 量级的时间相关差异）；静止且零多普勒的配置在入口就被拒。"),
        ...
        pitfalls=(
            ...
            "把每个样本的时间轴都从 t=0 重来，于是静止 UE 的多个样本逐位重复，却当成多个独立实现去算统计量。",
