...
DETAIL_SPECS.update({
    "raytracing": DetailSpec(
        ...
        principles=(
            ...
            "RT 的时间变化来自路径几何与完整速度向量。最大 Doppler 是 |v|/λ，每条路径再按速度方向和传播方向投影；若先把速度压成到最近站的径向分量再在路径层投影，会重复乘余弦。<strong>适配层直接按 slot 生成，不走 symbol 级中间层</strong>：时间轴是 <code>num_slots_per_sample</code> 个点、间隔 <code>sample_interval_s</code>，没有「先算 14 个 symbol 再抽中间那个」这一步。",
            "<strong>样本内部的时间轴从 0 起算，与 CDL 逐字一致。</strong>样本之间的差异只来自父类挪 UE 位置后 RT 重追的几何，<strong>不叠加任何额外相位</strong>。曾经试过让第 k 个样本从 <code>k × sample_interval_s</code> 起算（想让静止 UE 的多样本不再重复），那是错的：父类已经把位置推进了 <code>round × dt</code>，重追的径里已经含这段几何相位，再叠一次 Doppler 就是<strong>把移动相位算两遍</strong>——实测 Munich depth=0 单径几何相位步进 0.094718 rad、额外 Doppler 0.095166 rad、最终 0.189884 rad，等效 Doppler 接近翻倍，相干时间偏短，CSI 老化被系统性夸大。它同时把 <code>static</code> 偷偷改成了连续轨迹，与跨引擎合同冲突。",
            "<strong>确定性引擎注定重复时要硬失败，而且要拦全。</strong>RT 没有随机源，CDL 靠 <code>rng_small</code> 每轮重画小尺度实现这条路它走不了。三种配置要硬拦：① 父类实际不挪 UE（<code>static</code>，或非 static 但速度为 0）+ 多轮——几何固定、时间轴又都是 <code>[0, n_time×dt)</code>，每轮逐位重复；② <code>num_slots_per_sample&gt;1</code> + 零多普勒——时间相位恒为 1，样本内部 N 个 slot 逐位相同；③ UE 真正移动 + <strong>每个 UE 超过一轮</strong> + <code>num_slots_per_sample&gt;1</code>——父类每轮只前移<strong>一个</strong> <code>sample_interval_s</code> 而样本横跨 <code>n_time</code> 个间隔，相邻两轮窗口重叠（n_time=8 时重叠 7/8，16 个输出只有 9 个独特时刻），下游还会直接展平当独立快照。第 ③ 条要改的是父类的轨迹时钟、跨两个引擎，需要维护者定案；未定案前 RT 只拒绝这种多轮组合，<strong>单窗口（<code>num_samples&lt;=num_ues</code>）的移动多时隙是合法配置</strong>。",
        ...
        pitfalls=(
            ...
            "静止 UE 的多个样本逐位重复，却当成多个独立实现去算统计量——RT 是确定性引擎，这种配置只能拒，不能靠改时间轴糊过去。",
            "为了消除上面那个重复，给每个样本加一个「轮次时间原点」——父类挪位置带来的几何相位与它叠加，<strong>移动 UE 的相位被算两遍</strong>。",
            "用 <code>cfg.get(k, d) or d</code> 取射线追踪参数，把合法的 <code>rt_max_depth=0</code>（LOS-only 对照）悄悄换成 3。",
            "只把 <code>scene</code> 展开成 <code>scenario</code>/<code>osm_path</code> 而不把名字传给引擎，让非 munich 场景静默跑成 munich。",
