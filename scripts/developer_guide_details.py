...
DETAIL_SPECS: dict[str, DetailSpec] = {
    ...
    "architecture": DetailSpec(
        ...
        principles=(
            ...
            "时间也有三层语义。14 个 OFDM symbol 可以用于导频映射与信道估计，一个 channel snapshot 表示某个时刻可复用的物理状态，一个 TTI 则驱动队列、调度、BLER、OLLA 和 KPI。系统仿真在每个 TTI 引用一个 snapshot 并不会抹掉 symbol 级估计；它只是把已完成的物理计算压成系统层需要的状态，避免把 14 倍数据无意义地带进长时仿真。",
        ),
        implementation=(
    ...
    "channel": DetailSpec(
        ...
        implementation=(
            ...
            ("形成时间快照", "symbol 级数据用于估计链；适配器选择并记录 snapshot 规则，系统层按 <code>snap_every</code> 将 TTI 映射到物理状态。"),
        ),
        ...
        source_paths=("src/superran/channelhub.py", "src/superran/scenes.py", "src/superran/spec38901.py", "src/superran/generate.py"),
    ),
    "antenna": DetailSpec(
    ...
    "srs": DetailSpec(
        ...
        example=(
            ...
            "<p>导频每 4 RB 一个时，LS 在导频点无偏但中间 RB 依赖线性插值。LMMSE 若知道接近真实的 RMS delay spread，可用频域相关把多个导频联合加权；先验错得很大时，单条 realization 不保证优于 LS。因此验证看多样本 NMSE 分布和先验敏感性，而不是挑一个样本断言 LMMSE 必胜。</p>"
        ),
        checks=(
        ...
        source_paths=("src/superran/physical.py", "src/superran/channelhub.py", "src/superran/csi_aging.py"),
    ),
    "measurements": DetailSpec(
        ...
        source_paths=("src/superran/loader.py", "src/superran/measure.py", "src/superran/physical.py"),
    ),
...
DETAIL_SPECS.update({
    ...
    "mu": DetailSpec(
        ...
        implementation=(
            ...
            ("选择与落账", "SU 清空全队列则选 SU；否则 MU useful≥SU useful 才选 MU。执行后按真实模式更新队列、PF、OLLA、PRB 与配对 KPI。"),
        ),
        ...
        pitfalls=(
            ...
            "SU 已经清空所有包仍强制 MU，只为了提高 MU 配对比例。",
        ),
    ...
    "kpi": DetailSpec(
        ...
        principles=(
            ...
            "工作台是 experience_v2 的标准结果面，而不是运行结束后手工挑几张图。当前登记 22 项小区 KPI 与 20 项用户 KPI；可用项取决于 Result 是否真的携带相应数据。页面同时保留 95% CI、replication 数、KPI key、定义、告警、话务 profile 与校准轨迹，使一张卡片能向下追到统计样本和公式。<code>url</code> 只是便利入口，UTF-8 自包含 <code>html_path</code> 才是稳定离线产物；loopback 服务失败必须显式呈现，不能导致数值结果丢失或被悄悄替换。",
        ),
        implementation=(
            ...
            ("渲染标准工作台", "<code>render_html()</code> 用 selection 组装关注横幅、22 项小区/20 项用户注册表、CI 卡片、RBG 直方图、负载 gauge、逐 UE 图、经验 CDF、明细和定义折叠区。缺失字段只使对应卡片不可用，不伪造零值。"),
            ("落盘并提供入口", "<code>write_kpi_report()</code> 写入 artifacts/kpi 的 UTF-8 HTML，返回 html_path、url/serve_error、tabs、完整 selection 和支持清单。<code>sr_system_sim</code> 捕获呈现层异常并显式返回 error，同时保留已经完成的仿真 Result。"),
        ),
        ...
        source_paths=("src/superran/experience.py", "src/superran/kpi_view.py", "src/superran/results.py"),
    ),
    ...
    "rng": DetailSpec(
        ...
        implementation=(
            ...
            ("稳定汇总", "结果按 replication/drop/user key 排序并保存 seed lineage，paired analysis 只在相同 key 上做差。"),
        ),
...
DETAIL_SPECS.update({
    "gates": DetailSpec(
        ...
        source_paths=("src/superran/validate.py", "src/superran/gates.py", "src/superran/analysis.py", "src/superran/results.py"),
    ),
    "tests": DetailSpec(
        ...
        source_paths=("tests/", "scripts/run_developer_guide_qa.py", "tests/test_developer_guide.py"),
    ),
    "tools": DetailSpec(
...
DETAIL_SPECS.update({
    ...
    "csi": DetailSpec(
        ...
        principles=(
            "SRS 周期描述 UE 多久发送一次参考信号；CSI 陈旧时长描述调度时刻所用信道距其测量时刻多久；PMI/CQI 报告周期描述候选测量多久提交一次；channel snapshot 只是仿真器采样 H(t) 的网格。四者单位都可能是毫秒，却由不同事件驱动。把 snapshot=5 ms 误写成 PMI 每 5 ms 更新，会给基站额外信息并系统性高估波束命中率。",
        ...
        example=(
            "<p>假设信道在 0/5/10/15/20 ms 有快照，SRS 每 10 ms 发送、处理需 2 ms，PMI/CQI 每 20 ms 报告。0 ms 采样的 CSI 最早在 2 ms 可用于设计；系统在 0 ms report instant 没有未来 2 ms 的结果，仿真初始策略必须明确。10 ms 新 SRS 在 12 ms 可用，但若报告到 20 ms 才提交，15 ms PDSCH 仍保持上一份 PMI/CQI。</p>"
        ...
        source_paths=("src/superran/csi_aging.py", "src/superran/measure.py", "src/superran/physical.py", "src/superran/system.py", "tests/test_csi_aging.py"),
    ),
    "robust": DetailSpec(
...
DETAIL_SPECS.update({
    "agentloop": DetailSpec(
        ...
        source_paths=("src/superran/decisions.py", "src/superran/plan.py", "src/superran/algorithms.py", "src/superran/algo_defs.py", "src/superran/algo_defs2.py", "src/superran/spec.py", "src/superran/bridge.py"),
    ),
})
...
DETAIL_SPECS.update({
    "raytracing": DetailSpec(
        ...
        principles=(
            ...
            "上游资产必须只读。Mitsuba 对部分 VTK PLY 头中的 <code>obj_info</code> 不兼容，<code>prepare_scene()</code> 会按内容哈希复制到 SuperRAN 缓存并清理副本；源 PLY 的 mtime、大小和摘要保持不变。这样修复解析兼容性不会污染 ChannelHub，也不会让下一次实验悄悄使用一份被手改的城市模型。",
        ...
        implementation=(
            ...
            ("准备只读资产", "<code>prepare_scene()</code> 建立哈希缓存，复制 PLY 后只在副本删除不兼容 header；manifest 保存 source/cache 路径与变换说明。"),
        ...
        checks=(
            ("资产不变", "准备前后上游 PLY 的 SHA、大小和 mtime 不变，缓存副本可被 Mitsuba 读取且 provenance 完整。"),
        ...
        source_paths=("src/superran/scenes.py", "src/superran/scenario.py", "src/superran/channelhub.py", "src/superran/generate.py", "src/superran/loader.py", "tests/test_raytracing.py", "tests/test_interference.py"),
    ),
})
...
DETAIL_SPECS.update({
    "bler": DetailSpec(
        promise="把一次 TTI/TB 的公司误块事件、28 档 NewTx/ReTx 源曲线、TBS 查询轴、分析 CB→TB 后端和系统重传近似逐层分开。读完后应能回答：这次 ACK/NACK 对应哪个 TB、查了哪条曲线、实际使用了哪些输入，以及哪些维度仍没有被导入。",
        principles=(
            "38.214 规定的是 CQI/MCS 表、谱效率和 TBS 过程，不提供一套可直接套用所有接收机的“标准 BLER 曲线”。<code>BlerModel</code> 用有限码长与实现损失构造分析瀑布；<code>company_20b_256qam</code> 是特定接收机数据。两者都可以服务系统抽象，但报告必须带 backend/model_version，不能把分析模型或公司数据写成 3GPP 真值。",
            "公司口径已经确认：误块事件是<strong>一个已调度 TTI 中该用户的整个 TB</strong>，系统不另外暴露 CBLER。概念工作点包含该次 TBS、post-MMSE SINR、选定 MCS、NewTx/ReTx 与固定接收机 profile；一次查询只形成一次 TB ACK/NACK。物理编码内部即使分成多个 CB，也不应在公司表 3 路径上再次套 <code>1-(1-p)^C</code>。",
            "QAM 互信息首先依赖噪声约定。代码使用复基带平均能量归一与 σ²=1/γ；若把实维高斯求积尺度误写成 √(2γ) 等价形式而未匹配方差，会引入约 3 dB 偏移。缓存表应在低 SNR 接近 0、高 SNR 饱和于 log2(M)，并随 γ 单调，才能作为 MIESM 的可逆映射。",
            ...
            "当前导入数据与公司概念口径仍有一个明确缺口：<code>bler_data_20b.py</code> 只保存 MCS、NewTx/ReTx 码率和 SINR→BLER 点，没有把每档 reference TBS、RBG/rank、信道与译码 profile 结构化保存。运行时 <code>_bler_lookup(mcs, sinr)</code> 因而没有使用本次 TBS。正确说法是“TTI/TB 事件单位已对齐，但 TB-size 轴尚未对齐”，不能把 TBS 已计算误写成 TBS 已参与 BLER 查询。",
            "分析后端的 CB→TB 合成假设码块错误近似独立，TB BLER 为一减全部码块成功概率。它仅用于表 1/2；公司表 3 不区分 CB/TB 接口，因此不能读取同一条公司 TBLER 后再暗加一次 CB 合成。数据合同缺什么维度，就把它列为适用边界，不能从 MCS 名称猜出来。",
            "NewTx/ReTx 曲线也不等于标准 HARQ 状态机。legacy_v1 首次查 NewTx、后续失败查同一 ReTx 曲线，但不维护软缓冲累计；experience_v2 的 NACK 字节留在 FIFO，下一次仍按 NewTx 抽样。二者都可作为明确版本的近似，却不能声称实现 RV、IR/CC soft combining、HARQ process timing 或 buffer state。",
        ),
        implementation=(
            ...
            ("映射频选 SINR", "<code>effective_sinr()</code> 根据 modulation/method 选择 MIESM 或 EESM；EESM 接受显式 beta，缺正式标定时结果只能作为参考。"),
            ("计算分析 BLER", "表 1/2 按 MI 余量、码长和实现损失得到 CB 瀑布，再按 38.212 分段估算 C 并合成 TB BLER；anchor_check 只输出对标点。"),
            ("装载公司曲线", "<code>bler_data_20b.py</code> 提供 28 MCS×NewTx/ReTx 原始点，<code>verify_curves()</code> 检查 SHA、覆盖、横轴/BLER 单调和 10% crossing，并同时暴露 TTI/TB 事件合同与 TB-size 轴缺口。"),
            ("形成一次 TTI/TB 判错", "调度器先确定 grant、rank、MCS 与 TBS，再以真值 SINR查公司曲线并从独立 BLER 随机流抽一次 ACK/NACK；CB 不进入系统状态。"),
            ("进入系统模式", "capacity/legacy 首传用 NewTx、重传用 ReTx；experience 的 NACK payload 留队后仍按 NewTx。两者都必须记录实际 TBS、curve backend、transmission kind 与缺失的 TB-size 映射。"),
        ),
        example_title="同一 MCS 下 1 RBG 与 17 RBG：事件单位对了，块长轴仍没用上",
        example=(
            "<p>当前 TBS 实算中，MCS12/rank2 的 1 RBG 是 1,729 B，17 RBG 是 29,722 B。公司语义下，两者各自仍然只是“这个 TTI 的一个 TB”并各抽一次 ACK/NACK；不能因为物理分段函数分别算出 2 与 29 个 CB，就在表 3 上额外做 CB 合成。</p>"
            "<p>但当前 <code>experience._bler_lookup(mcs, sinr)</code> 对两者会查询同一条 MCS12 曲线：只要 SINR 相同，BLER 就完全相同，29,722 B 并没有进入函数。这不是公司不看 TB size，而是导入数据尚未保存 reference TBS 映射。现在只能把它标成近似；补齐映射后再决定按离散 TBS profile 查表还是按经标定的块长模型插值。</p>"
            "<p>同时，等效 SINR 必须位于数据所有者确认的经典 MMSE post-processing 参考面；换成 IRC/RZF 或直接使用天线前 Es/No，也会让曲线失配。</p>"
        ),
        checks=(
            ("MI 锚点", "各 QAM 的 I(γ) 单调、范围为 0..log2(M)，逆映射往返误差受控；独立 Monte Carlo/高精度积分反查噪声尺度。"),
            ("有效 SINR 退化", "全元素相等时 MIESM/EESM 精确返回原 SINR；深衰 toy case 的方向与理论一致，beta 变化可解释。"),
            ("TB 合成", "C=1 时 TB=CB，C 增大时 TB BLER 不下降，概率始终有限且无 NaN。"),
            ("公司数据完整", "固定摘要、28 MCS、56 curves、1,824 points、单调横轴/BLER 与 crossing 全部验证。"),
            ("全曲线可见", "手册 28 行审计表与 56 条瀑布由源常量直接生成；10% crossing、源范围和点数与 verify_curves 一致。"),
            ("参考面与事件标签", "查询结果带 source axis、receiver、one-TTI/TB event、tx kind、clamp 状态和 TB-size 轴状态；分析后端不伪装公司后端。"),
            ("系统边界可见", "experience 结果明确当前未使用 MIESM/EESM、TBS 尚未进入曲线查询、NACK 再调度仍标 NewTx；legacy ReTx 无软合并声明。"),
        ),
        pitfalls=(
            ...
            "使用未标定 EESM beta 生成正式公司性能结论。",
            "对公司曲线范围外做无依据外推，或把 MMSE SINR 曲线用于不同接收机参考面。",
            "把公司一 TTI/TB 事件错误地展开为独立 CBLER，导致同一源曲线被重复合成。",
            "看到系统已经计算 TBS，就误以为当前 BLER 查询已经把 TBS 当输入。",
            "擅自把 <code>20B</code> 扩写成字节数、带宽或其他配置；该标识的确切展开与 reference profile 尚未结构化保存。",
            "把一条 ReTx 曲线复用称为完整 HARQ soft combining。",
        ),
...
DETAIL_SPECS.update({
    "externalresults": DetailSpec(
        ...
        principles=(
            ...
            "预注册锁的是分析身份，不是阻止研究迭代。<code>analysis.lock()</code> 对规范化 JSON 计算 SHA-256，每次改 baseline、primary metric、方向、样本规则或排除条件都会产生新文件；已经绑定到数据集的摘要不可原地改写。探索结果仍可计算，但分类为 exploratory/secondary，不能在看完结果后把最有利指标改名为预注册 primary。",
        ...
        implementation=(
            ...
            ("绑定数据生成", "<code>sr_generate</code> 在 summary/manifest 中保存 prereg_id 与 digest；计划变化必须重新锁定并生成新身份，不能回填旧数据。"),
            ("导出评测模板", "<code>deliver.build_code()</code> 生成读取 Dataset、按 h_est 设计、按 h_true 评价并写 ResultArtifact 的模板；用户可替换算法主体但保留合同。"),
            ("注册逐样本结果", "<code>ResultArtifact</code> 验证有限值、sample_ids 长度/唯一性、metric/unit 与数据集摘要，写 values.npz、manifest 和 SHA；可选记录 code hash/method metadata。"),
        ...
        example=(
            ...
            "<p>另一种事故是同名 dataset_id 的 channels.npz 被重新生成。sample_ids 仍相同，却对应不同信道；dataset_digest 会阻断。只有摘要、顺序、指标、单位、方向和 prereg identity 全部一致，CRN 才能让逐样本差值隔离算法效应。外部脚本声明使用 h_est 仍只是声明，最终报告会保留这项审计限制。</p>"
        ),
        checks=(
...
FORMULA_SPECS: dict[str, FormulaSpec] = {
    ...
    "F_SRS_RX": FormulaSpec(
        "SRS 接收观测模型",
        "64 根基站接收端口在 SRS 资源上看到目标 UE 的 4 端口导频、同频干扰和热噪声。去扩频/估计的输入是 Y，而不是直接访问 H。",
        (("Y<sub>SRS</sub>[k]", "第 k 个 SRS 频域资源上的基站接收矩阵。"),
         ("H<sub>UL</sub>[k]", "该频点真实上行信道，shape 为 64×4。"),
         ("X<sub>SRS</sub>[k]", "4 个 UE 端口发送的已知 SRS 序列/导频矩阵。"),
    ...
    "F_SRS_LAG": FormulaSpec(
        ...
        "调度时刻只能使用最后一次已经到达并完成处理的 SRS 报告。连续时间 lag 先换算成 snapshot 步数，再回看物理信道；max 防止仿真开头访问负索引。",
        (("τ<sub>b</sub>(t)", "波束/报告 b 在当前时刻 t 的等效 CSI 时延。"),
         ("t<sub>last SRS,b</sub>", "报告 b 最近一次 SRS 发送/采样时刻。"),
         ("D<sub>proc</sub>", "接收、估计、上报到调度可用的处理时延。"),
    ...
    "F_TX_SINR": FormulaSpec(
        "SU 发送侧 MCS 所依据的 SINR 预测",
        "CQI 先映射到一个门限/基础 MCS 对应的 SINR，再加波束赋形增益和 SU OLLA 修正。它是 gNB 的发送侧预测，不等于接收后用真值计算的实际 SINR。",
        (("γ<sub>tx,SU</sub>", "SU 调度时用于选 MCS 的发送侧预测 SINR，单位 dB。"),
         ("Γ(MCS(CQI))", "CQI 经 MCS 映射后对应的基准 SINR 门限。"),
         ("G<sub>BF</sub>", "基于当前可见 CSI 预测的波束赋形增益，单位 dB。"),
         ("Δ<sub>OLLA</sub>", "该用户独立维护的 SU 外环链路自适应偏置。")),
    ),
    "F_MU_SINR": FormulaSpec(
        "MU 发送侧 MCS 比 SU 多出的三项",
        "MU 从同一 SU 基准出发，加入用户配对残留相关性损失、流/用户平分功率损失和该用户自己的 MU OLLA。损失项通常为负 dB；SU 与 MU OLLA 分开更新。",
        (("γ<sub>tx,MU</sub>", "MU 调度时用于选该用户 MCS 的发送侧预测 SINR。"),
         ("Γ(MCS(CQI))", "CQI 给出的 SU 基准 SINR 门限。"),
         ("G<sub>BF</sub>", "基于可见 CSI 的波束赋形增益。"),
         ("Δ<sub>SU OLLA</sub>", "该用户 SU 状态下积累的基础外环偏置。"),
         ("L<sub>corr</sub>", "配对用户残留相关性/零陷不完美造成的 dB 损失。"),
         ("L<sub>power</sub>", "MU 同资源上总功率在更多流之间平分造成的 dB 损失。"),
         ("Δ<sub>MU OLLA</sub>", "该用户在 MU 传输 ACK/NACK 上单独维护的附加外环偏置。")),
    ),
    ...
    "F_OLLA": FormulaSpec(
        ...
        (("Δ(t)", "当前用户在时刻 t 的 OLLA dB 偏置。"),
         ("Δ(t+1)", "处理本次 ACK/NACK 后的偏置。"),
         ("1<sub>ACK</sub> / 1<sub>NACK</sub>", "本次结果对应的 0/1 指示函数。"),
         ("s<sub>↑</sub> / s<sub>↓</sub>", "ACK 上调步长 / NACK 下调步长，单位 dB。"),
         ("clip", "把偏置限制在配置的 olla_min_db 与 olla_max_db。")),
    ),
...
FORMULA_SPECS.update({
    ...
    "F_RESULT_CONTRACT": FormulaSpec(
        ...
        (("d<sub>D</sub><sup>A/B</sup>", "A/B 结果绑定的 channels.npz 内容摘要。"),
    ...
    "F_EESM": FormulaSpec(
        ...
        "EESM 通过指数平均强化低 SINR 样本的影响。beta 决定映射曲率，必须按 MCS/链路曲线标定；当前库的按调制默认值只是参考，不能冒充公司链路标定。",
    ...
    "F_TB_BLER": FormulaSpec(
        "多个码块合成一个传输块错误率",
        "若一个 TB 被分成 C 个近似独立码块，只有全部码块正确时 TB 才成功，因此 TB BLER 是一减去全部成功概率。这一式只属于 SuperRAN 的表 1/2 分析后端；公司口径直接把一调度 TTI 的 TB 作为误块事件，不再从 CBLER 合成。",
    ...
    "F_COMPANY_TTI_BLER": FormulaSpec(
        "公司口径：一次调度 TTI 的 TB 就是一次 BLER 事件",
        "调度确定本 TTI 的资源、TBS 与 MCS 后，以该 TB 的 post-MMSE SINR 查询误块概率，再用独立随机数形成一次 ACK/NACK。公司系统不额外观察 CB 错误；但概念输入包含 TB size，不代表当前导入曲线已经保存了这一轴。",
        (("p<sub>t</sub><sup>TTI/TB</sup>", "第 t 个已调度 TTI 中该用户 TB 的误块概率。"),
         ("E<sub>TB,t</sub>", "该 TTI 的 TB CRC 失败事件；不拆成可见的 CB 事件。"),
         ("A<sub>t</sub>", "该次调度实际 transport block size，单位 bit。"),
         ("γ<sub>t</sub>", "与曲线同参考面的经典 MMSE post-processing SINR。"),
         ("m<sub>t</sub>", "本 TTI 选定的 MCS index。"),
         ("x<sub>t</sub>", "传输类型，当前曲线集为 NewTx 或 ReTx。"),
         ("ρ", "固定的接收机、资源、rank、信道与译码配置 profile。"),
