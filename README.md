...
- **CDF 话务与目标负载校准** —— 已内置 `experience_baseline_v1` 固定下行基线：4,274 点包大小 CDF（byte）+ 3,307 点包间隔 CDF（原始 value 单位为秒），两者用独立命名随机子流抽样。`packet_size_scale` / `interarrival_scale` 只乘各自 value 横轴，概率轴不变；支持一业务一对 CDF、多 profile 与 `ue_ids` 显式用户映射。`target_prb_utilization=0.30` 可用公共随机数调话务，最后另跑正式重复实验，未达容差绝不回填目标值；两张输入 CDF 图由同一文件自动生成，不冒充 PRB 或性能结果
