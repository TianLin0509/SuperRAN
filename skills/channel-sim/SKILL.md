...
56 条 NewTx/ReTx 解调曲线（1824 点），不是 3GPP 标准曲线；源横轴名为 Es/No，
本工具只在显式选择 `mcs_table=3` 时把它按有效 SINR 使用，并在结果里保留口径。
...
复用它并显式标成假设。严格跨接收机、跨块长、跨链路配置的 BLER 结论仍需真正的
链路级仿真（Sionna PHY / MATLAB 5G Toolbox）。
...
| "表 3 算出 0.1%，所以所有配置都可靠" | 曲线只代表源数据隐含的接收机/块长配置，跨配置要链路级复核 |
