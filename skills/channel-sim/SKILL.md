...
**配置敲定后调 `sw_spec_sheet(draft_id=...)`。它会自己在浏览器里打开。**
这是这一步的交付物：一份 HTML，画出阵列、拓扑、频域、时域、信道剖面，
外加一张逐项标注"用户指定 / 系统默认"的参数全表，以及一个可交互的调参面板。
把 `headline` 和 `notes` 转述一句，**不要把图或 HTML 内容贴回对话**。

返回的 `writeback == "post"` 时，紧接着调 **`sw_await_config()`** 等用户：
他在页面上改完点「应用到仿真」，改动**直接回到你手里**，不用他复制粘贴。
`got=0` 只是这段时间没点，不是错误——他还在看就再等一轮，
他已经在对话里说话了就按对话来。
`writeback == "clipboard"` 时（服务没起来，原因在 `serve_error`）
才退回"让他复制、粘回对话框"。
