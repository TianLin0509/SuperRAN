...
换来的小包即时服务比例只从 0.5235 升到 0.5280（+0.45 pp）—— 因为按需 RBG 分配早就让
小包在 PF 下也基本即时服务。PRB 利用率 30% 时四种调度器的小区吞吐与 Jain **完全相同**，
只剩 1.4 个百分点的小包即时服务差异（0.7243 → 0.7380）。
以上为 2026-09-03 在下行 AMC 链修正后的基线上重测；旧基线上是 0.4707→0.3032、2 个饿死、
0.7665→0.7891（+2.3 pp）。修正后收益更小而代价更大，结论方向不变且更强。
**EDF 只在拥塞时才有意义。**
...
无量纲比值，实测中位数相差约 836 倍，`s=1.0` 时名义 `w=0.5` 的实际 EDF 占比只有
0.0012。结果里的 `cell.scheduler_mixed_component_scale.effective_edf_share` 报出真实占比，
...
饿死归零，代价是小区吞吐相对纯 EDF −23.8%（423.1 → 322.3 Mbps）、小包即时服务掉到
0.5161（**比 PF 基线的 0.5235 还低**）；100 ms 门限更退化成纯"最久等待优先"，
小包即时只剩 0.4457。标定 `edf_mixed_epf_scale` 到分量可比（该场景 1e-4）后，
混合模式零饿死、吞吐 429.2 Mbps、小包即时 0.5229，在每个轴上都优于硬兜底。
以上为 2026-09-03 在下行 AMC 链修正后的基线上重测。
...
`SystemConfig.cqi_report`，默认 `CqiReportConfig(srs_period_tti=4,
srs_delay_tti=3, ue_implementation_loss_db=1.5)`。两条主循环都走：每
`srs_period_tti` 个 TTI 让 UE 上报一次，测的是 `tti − srs_period_tti −
srs_delay_tti` 时刻的信道（扣掉 UE 实现损失），IIR 在线更新，基站读的时候
再按**当前快照**把瞬时 BF Gain 加回去。`enabled=False` 退回建表阶段一次性
算好的 `sinr_tx_db`，**逐位一致**。

**它和 `csi_aging` 是两个维度**：`csi_aging` 管预编码权 `h_prec` 的陈旧度，
本项管 MCS 决策输入 `sinr_tx_db` 的更新频率，误块抽签用的 `h_eval` 真值两者
都不碰。结果里带 `cqi_update_count_mean` 与 `cqi_age_tti_max` 两个新鲜度诊断
（关闭时为 `null`）。

三条读数字之前必须知道的事：

1. **离线那份 AMC 坐标系统性乐观。** 同一夹具 OLLA 关掉时，离线预计算首传
   BLER 0.44（目标 0.1），理想 CQI（周期 1 / 时延 0 / 无损失）0.04。
...
做 OLLA 稳态实验时先确认两臂都没顶到钳位，否则比的是钳位值不是稳态偏置。
