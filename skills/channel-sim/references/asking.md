...
每个方括号是一个必须填的槽。**空着的槽就是该问的问题。** `sr_missing_slots`
按"空着的代价"从大到小返回，每个槽自带 3~4 个选项。

`sr_plan` / `sr_revise` 把该问的打包成一轮，两者取其一即可——
不确定用哪个时用 `sr_plan`，它还会给出扫描建议和陷阱提示。
...
> ② 理想 CSI 下的逐 RB 特征预编码 —— 乐观参考；真正容量上界另算注水容量
> ③ 某篇已发表方法 —— 告诉我哪篇，我尽量对齐它的实验设置
> ④ 还没定，先看可行性
>
> 或者你直接说。

开放式问"你要跟什么比"，很多人第一反应答不上来；但看到"理想 CSI 特征预编码参考"
就会想起来还有这条路。
...
>
> **你**：（`sr_missing_slots` → 结论模板缺 4 个槽）先对齐三件事，这比参数重要。
>
> **① 跟什么比？** ① 理想 CSI 的逐 RB 特征预编码 **← 推荐** ② Type I/II 码本 ③ 现网 DFT 波束 ④ 还没定
>
...
> 或者你直接说。
>
> **用户**：跟理想 CSI 的逐 RB 特征预编码比，预期 3~10%，先用理想信道。
>
> **你**：（`sr_revise` → 第 2 轮参数 → `sr_generate` 20 个试点 → `sr_gate`）
...
> **用户**：补。
>
> **你**：（`sr_generate` 64 → `sr_gate` → `sr_compare_arms`）
> 门 1、门 2、门 3 全过。你的方案 28.41 vs 理想 CSI 特征预编码 30.53 bit/s/Hz，
> 差值 -2.12（-6.9%），95% CI [-2.71, -1.53]，n=64，p=1.2e-9。
> 达到该参考的 93%。这是相对理想 CSI 等功率特征波束的结果；不要写成“容量上界的
> 93%”，真正的 Shannon 上界需用逐时频注水容量另算。
