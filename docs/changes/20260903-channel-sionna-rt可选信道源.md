...
* **不是逐位可复现**。Sionna 的 PathSolver 即使固定 seed、单线程、同进程连跑也有
  末位差异；传到信道上重跑 NMSE −65~−122 dB。RT 数据集不能靠重跑逐位复算，
  worker 切片也只在这个量级一致。meta 里 `rt_bit_reproducible=False` 显式声明。
* **服务小区被挡的比例没有系统评估**。实测 7 站 munich 下 1/6 的 UE 出现「38.901
  选的服务小区 RT 追不到径、但别的小区可达」，只在这一个配置上量过。
* **阵元方向图没有逐径加权、没有 beam squint**。实测服务向扇区径间相对权重被高估
  0.2~2.6 dB（中位 1.1 dB）；干扰扇区的角度散布更大，未测量。
